...
- **编辑模式（上帝视角，可滚动画布 + 交替网格 + 详情栏）**：绝对定位画布渲染，行与列均按「连接块 · 地块 · 连接块」交替排布——奇数轨为地块（正方形，尺寸更大以利阅读名字，只显示名字与 id + 出生点徽标，layout 整数坐标决定主位），偶数轨为连接块（长方形，更窄，横向/纵向），偶数-偶数交叉处为无意义的小占位格（以斜纹弱化样式标示「无效占位、不可交互」）；**连接只画方向线条**（连接块内 SVG 方向条：单向箭头 / 双向双箭头 / 空块=不连接，不画方向文字）；**非相邻连接（目的地可自选）以虚线强调条显示在出发方向的连接块内**，替代原「地块分身」概念（无展开/收起按钮）——环路 / 捷径 / 跨区连接均以此呈现；地块永远真名、不存在 `???`。**可滚动画布 + 缩放**：画布尺寸 = 内容边界 + 四周固定留白（空白延伸有限，只有新增地块驱动画布生长），Ctrl/⌘+滚轮（含触控板捏合）以光标为中心缩放 + 工具条 − / + / 百分比 / 适应（20%–400%）；视口与内容无交集时自动回到内容。**右侧详情栏**（可收起/展开）：点击地块、方向条或空地块在栏内查看/编辑——**查看 / 编辑子模式**：查看模式只读；编辑模式点击空地块（含内容边界外的 HALO 圈）或网格背景新建地块、点击地块创建连接（方向 + 目的地可自选，方向指向相邻地块时自动选中该目标、否则可手动任选 → 特殊连接）、点击已存在的连接编辑（to/label/「隐藏目的地」开关/direction 四选/删除+确认）、点击有连接的连接块可编辑块内出口——**创建连接只从地块发起（延伸出去），空连接块不可点击**。
