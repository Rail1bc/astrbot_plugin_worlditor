...
- **编辑模式（上帝视角，交替网格表格 + 详情栏）**：行与列均按「连接块 · 地块 · 连接块」交替排布——奇数轨为地块（正方形，只显示名字与 id + 出生点徽标，layout 整数坐标决定主位），偶数轨为连接块（长方形，横向/纵向），偶数-偶数交叉处为无意义的小占位格；**连接只画方向线条**（连接块内 SVG 方向条：单向箭头 / 双向双箭头 / 空块=不连接，不画方向文字）；**非相邻连接（目的地可自选）以虚线强调条显示在出发方向的连接块内**，替代原「地块分身」概念（无展开/收起按钮）——环路 / 捷径 / 跨区连接均以此呈现；地块永远真名、不存在 `???`。**右侧详情栏**（可收起/展开）：点击地块、连接块、方向条或空地块在栏内查看/编辑——**查看 / 编辑子模式**：查看模式只读；编辑模式点击空地块新建地块、点击地块创建连接（方向 + 目的地可自选，方向指向相邻地块时自动选中该目标、否则可手动任选 → 特殊连接）、点击已存在的连接编辑（to/label/「隐藏目的地」开关/direction 四选/删除+确认）、点击连接块可补建该走向的连接（方向固定为该块的走向）。
