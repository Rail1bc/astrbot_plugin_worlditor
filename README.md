...
- **编辑模式**（上帝视角，无限画布 + 交替网格表格 + 详情栏）：行与列均按「连接块 · 地块 · 连接块」交替排布——地块为正方形（只显示名字与 id + 出生点徽标），连接块为长方形，交叉处为无意义的小占位格；**连接只画方向线条**（单向箭头 / 双向双箭头 / 空块=不连接）；**非相邻连接（目的地可自选）以虚线强调条显示在出发方向的连接块内**（替代原「地块分身」，无展开/收起按钮）；**可滚动画布**：内容四周固定留白（空白延伸有限，新增地块才扩展画布）、点击网格背景可就近新建地块；**地图缩放**：Ctrl/⌘+滚轮（或触控板捏合）以光标为中心缩放，工具条提供 − / + / 百分比 / 适应；右侧详情栏可收起/展开——**查看 / 编辑子模式**：查看模式只读；编辑模式点击空地块可新建地块、点击地块可创建连接（方向 + 目的地可自选）、点击已存在的连接可编辑、点击连接块可补建该走向的连接。
