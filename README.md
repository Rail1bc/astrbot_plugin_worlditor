...
- **玩家模式**：模拟玩家视角——地图中间是只含地块名称的小块，上下左右直接连接 1 跳可达地块（十字布局，目标格只显示地块名）；地图 div 动态填充剩余空间、视口内整体不滚动，说明文本在平级详情区（PC 左侧地图、右侧详情一整列；移动端上地图、下详情，详情列单独滚动）；隐藏目标显示 `???`，点击目标格按 `exit_id` 移动。
