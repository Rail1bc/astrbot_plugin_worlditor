...
- **编辑模式**（上帝视角，固定边界地图 + 右键拖动 + 缩略图 + 详情栏）：地图为**固定大小网格**——内容边界外任意方向再延伸 10 个空地块（视图有大小，可平移范围固定）；**地块是唯一的「格」**（正方形，只显示名字与 id + 出生点徽标），连接不再以格呈现，而是绘制在地块间隙中的连线——**a→b 单向**带箭头、**a↔b 双向**无箭头连线、**a→b′ 非常规连接**（目的地非方向相邻）带箭头虚线 + 末端小标记格；相邻地块之间可同时显示两条连接（如 a→b 普通 + b→c′ 非常规，各自独立绘制）；**查看 / 编辑子模式**：查看模式只显示已存在的地块与连接（无空地块格），编辑模式显示全部空地块（点击新建）与网格背景；**视图隐藏横竖滚动条**：右键拖动 / 滚轮平移，Ctrl/⌘+滚轮以光标为中心缩放，工具条提供 − / + / 百分比 / 适应；**右下角全图缩略图**：显示全图与当前视口范围，可收起/展开、可拖动，点击或拖动可跳转视口；右侧详情栏可收起/展开——点击地块、连接线或空地块查看/编辑，创建连接从地块面板发起（方向 + 目的地可自选）。
- **玩家模式**：模拟玩家视角——地图中间是只含地块名称的小块，上下左右直接连接 1 跳可达地块（十字布局，目标格只显示地块名且收缩到内容大小，长名在格内换行）；地图 div 动态填充剩余空间、视口内整体不滚动，说明文本在平级详情区（PC 左侧地图、右侧详情一整列；移动端上地图、下详情，详情列单独滚动）；隐藏目标显示 `???`，点击目标格按 `exit_id` 移动。
